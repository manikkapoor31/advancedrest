    ...
    if(req.params.authToken||req.query.authToken||req.body.authToken||req.header('authToken')){
        Auth.findOne({authToken:req.header('authToken')||req.params.authToken||req.body.authToken||req.query.authToken},(err,authDetails){
            if(err){
                ...
                jwt.verify(authToken,authDetails.tokenSecret,(err,decoded)=>{
                    if(errr){
                        if(err.name='TokenExppiredError'){
                            logger.error(err.message,'Authorization Middleware',10)
                            let apiResponse=responseLib.generate(true,'Token is Expired , Login Again',500,null)
                            res.send(apiResponse)
                        }
        ...
        let apiResponse=responseLib.generate(true,'Authentication Token Missing in Request',500,null)
        res.send(apiResponse)
    }
}
